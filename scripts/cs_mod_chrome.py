...
"""A standalone script to read and parse the Chrome history database for each
user on disk.
"""
...
parser = argparse.ArgumentParser(description='Read and parse Chrome history database.')
parser.add_argument('--tmp', default=os.path.expanduser('~/tmp/cs.chist.' + datetime.now().strftime('%Y%m%d') + '/'), help='Temporary directory')
parser.add_argument('--profile', default='Default', help='Chrome profile name')
args = parser.parse_args()

# Set variables with default values or from command-line options
